...
class DouyinApi(object):
    def __init__(self):
        self.urls = Urls()
        self.result = Result()
        # Used to set maximum time for repeating request to an interface
        self.timeout = 10

    # Extract URL from sharing link
    def getShareLink(self, string):
        # findall() find matching strings in regex
    ...
    def getKey(self, url):
        ...
        # Douyin updated collection to note
        # Work Step 1 parsed link is share/video/{aweme_id}
        ...
        # User Step 1 parsed link is share/user/{sec_uid}
        ...
        if key is None or key_type is None:
            print('[  Error  ]: Input link is incorrect! Unable to get ID\r')
            return key_type, key
